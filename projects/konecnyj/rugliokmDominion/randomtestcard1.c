...
void checkMine(struct gameState* pre)
{
	...
	*/
	...
	{
		areValidInputs = 1;
		asserttrue(cardEffect(mine, handpos + 1, randGetCard, 0, &post, handpos, 0) == 0, "cardEffect returns 0");
	}
	else
	{
		asserttrue(cardEffect(mine, handpos + 1, randGetCard, 0, &post, handpos, 0) != 0, "cardEffect does not return 0");
	}
